...
// SQL DIFF ...
func diff(sourceDbConfig *Mysql, targetDbConfig *Mysql, sourceDb *gorm.DB, targetDb *gorm.DB, sourceSchema Schema, sourceTable Table, targetTableMap map[string]Table) {
	defer wg.Done()
...
// CREATE TABLE ...
func createTable(sourceDbConfig *Mysql, sourceDb *gorm.DB, sourceSchema Schema, sourceTable Table) {
	var (
	...
	sourceDb.Table("COLUMNS").Order("`ORDINAL_POSITION` ASC").Find(
		...
		sourceDbConfig.GetDb(), sourceTable.TableName,
	)
	...
	if sourceColumnDataLen > 0 {
		...
		if sourceStatisticsLen > 0 {
			...
			for _, sourceStatistic := range sourceStatisticsData {
				...
				if xutil.InArray(sourceStatistic.IndexName, sourceStatisticIndexNameArray) {
...
// ALTER TABLE ...
func alterTable(sourceDbConfig *Mysql, targetDbConfig *Mysql, sourceDb *gorm.DB, targetDb *gorm.DB, sourceTable Table, targetTableMap map[string]Table) {
	...
	targetDb.Table("COLUMNS").Order("`ORDINAL_POSITION` ASC").Find(
		&targetColumnData,
		"`TABLE_SCHEMA` = ? AND `TABLE_NAME` = ?",
		targetDbConfig.GetDb(), sourceTable.TableName,
	)
...
func createView(sourceDbConfig *Mysql, targetDbConfig *Mysql, sourceDb *gorm.DB, targetDb *gorm.DB, sourceTable Table, targetTableMap map[string]Table) {
	var (
		sourceView View
		targetView View
	)
	...
	sourceView.ViewDefinition = strings.Replace(sourceView.ViewDefinition, fmt.Sprintf("`%s`.", sourceDbConfig.GetDb()), "", -1)
	...
	if _, ok := targetTableMap[sourceTable.TableName]; ok {
		...
		targetView.ViewDefinition = strings.Replace(targetView.ViewDefinition, fmt.Sprintf("`%s`.", targetDbConfig.GetDb()), "", -1)
